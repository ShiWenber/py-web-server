...
def do_post(client, request_message, url):
    ...
    try:
        ...
        if url == b"/test":
            ...
            response_header = bytes("Server:My server\r\n", "utf—8")
            # 添加内容类型，浏览器才能按照对应方式解析
            response_header += bytes("Content-Type:text/html\r\n", "utf-8")
            postdict = to_postdict(entity_body(request_message))
            # response_body = bytes(str(postdict), "utf-8")
            # for key, value in postdict.items():
                # response_body += key + b" = " + value + b"\n"
            # 画出HTML表格
            temp = bytes("<table border='1'><tr><th>key</th><th>value</th></tr>", "utf-8")
            for key, value in postdict.items():
                temp +=b"<tr><td>" + key + b"</td><td>" + value + b"</td></tr>"
            temp = temp + bytes("</table>", "utf-8")
            response_body += temp
            ...
            response_start_line = bytes("HTTP/1.1 200 OK\r\n", "utf-8")
            response_header = bytes("Server:My server\r\n", "utf—8")
            response_body = bytes("upload", "utf-8")
...
def do_get(client, request_message, url, file_name):
    ...
    try:
        ...
        response_header = bytes("Server:My server\r\n", "utf-8")
        response_header = bytes("Content-Type:text/html\r\n", "utf-8")
        file_404 = open(HTML_ROOT_DIR + "/github404.html", "rb")
        file_404_data = file_404.read()
        file_404.close()
        response_body = file_404_data
